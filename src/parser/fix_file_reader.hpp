...
public:
    ...
    bool OpenNextFile(FileSystem &fs, const vector<string> &files, 
                     idx_t &file_index, std::mutex &lock);
    ...
    const string& GetCurrentFile() const { return current_file_; }
    
    // Get current line number (1-indexed)
    idx_t GetLineNumber() const { return line_number_; }
