...
int FixGroupParser::GetGroupCount(
...
size_t FixGroupParser::FindCountTagPosition(
...
vector<Value> FixGroupParser::ParseGroupInstances(
    const std::vector<std::pair<int, ParsedFixMessage::TagValue>> &ordered_tags,
    size_t start_pos,
    int group_count,
...
Value FixGroupParser::ParseGroups(
    ...
    if (parsed.all_tags_ordered.empty() || 
        parsed.msg_type == nullptr || 
        parsed.msg_type_len == 0) {
        return Value();  // NULL
    }
    ...
    for (const auto& [count_tag, group_def] : message_def.groups) {
        ...
        auto group_instances = ParseGroupInstances(
            parsed.all_tags_ordered,
            count_tag_pos + 1,
            group_count,
            group_field_tags
        );
