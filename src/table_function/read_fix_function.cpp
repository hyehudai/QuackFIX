...
struct FixColumnWriter {
    ...
    FixColumnWriter(DataChunk &out, idx_t row, const ReadFixBindData &bind, 
                   const ReadFixGlobalState &gs, vector<string> &errors)
...
void FixColumnWriter::WriteTagsMap(const ParsedFixMessage &parsed) {
    ...
    if (out_idx == DConstants::INVALID_INDEX) return;
...
void FixColumnWriter::WriteCustomTags(const ParsedFixMessage &parsed) {
    for (size_t i = 0; i < bind_data.custom_tags.size(); i++) {
        ...
        if (out_idx == DConstants::INVALID_INDEX) continue;
        ...
        switch (tag_num) {
            case MSG_TYPE:        value_ptr = parsed.msg_type; value_len = parsed.msg_type_len; break;
            case SENDER_COMP_ID:  value_ptr = parsed.sender_comp_id; value_len = parsed.sender_comp_id_len; break;
            case TARGET_COMP_ID:  value_ptr = parsed.target_comp_id; value_len = parsed.target_comp_id_len; break;
            case MSG_SEQ_NUM:     value_ptr = parsed.msg_seq_num; value_len = parsed.msg_seq_num_len; break;
            case SENDING_TIME:    value_ptr = parsed.sending_time; value_len = parsed.sending_time_len; break;
            case CL_ORD_ID:       value_ptr = parsed.cl_ord_id; value_len = parsed.cl_ord_id_len; break;
            case ORDER_ID:        value_ptr = parsed.order_id; value_len = parsed.order_id_len; break;
            case EXEC_ID:         value_ptr = parsed.exec_id; value_len = parsed.exec_id_len; break;
            case SYMBOL:          value_ptr = parsed.symbol; value_len = parsed.symbol_len; break;
            case SIDE:            value_ptr = parsed.side; value_len = parsed.side_len; break;
            case EXEC_TYPE:       value_ptr = parsed.exec_type; value_len = parsed.exec_type_len; break;
            case ORD_STATUS:      value_ptr = parsed.ord_status; value_len = parsed.ord_status_len; break;
            case PRICE:           value_ptr = parsed.price; value_len = parsed.price_len; break;
            case ORDER_QTY:       value_ptr = parsed.order_qty; value_len = parsed.order_qty_len; break;
            case CUM_QTY:         value_ptr = parsed.cum_qty; value_len = parsed.cum_qty_len; break;
            case LEAVES_QTY:      value_ptr = parsed.leaves_qty; value_len = parsed.leaves_qty_len; break;
            case LAST_PX:         value_ptr = parsed.last_px; value_len = parsed.last_px_len; break;
            case LAST_QTY:        value_ptr = parsed.last_qty; value_len = parsed.last_qty_len; break;
            case TEXT:            value_ptr = parsed.text; value_len = parsed.text_len; break;
            default:
